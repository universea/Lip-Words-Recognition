...
def video_loader(frames, nsample, seglen, mode):
    videolen = len(frames)
    cycle_times  = int(nsample / videolen)
    pre_pictrues = nsample % videolen
    imgs = []
    
    step = int(nsample / videolen)  # 每张图复制几次
    great = int(nsample % videolen)  # 前几张图多复制一次

    for idx in range(videolen):
