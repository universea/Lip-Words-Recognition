...
def test_with_pyreader(test_exe,
    ...
    try:
        ...
        acc1 = test_metrics.finalize_and_log_out("[TEST] Finish")
    finally:
        test_pyreader.reset()
    return acc1
...
def train_without_pyreader(exe, train_prog, train_exe, train_reader, train_feeder, \
    ...
    for epoch in range(epochs):
        ...
        save_model(exe, train_prog, save_dir)
    ...
    for epoch in range(epochs):
        ...
        try:
            ...
            if test_exe and valid_interval > 0 and (epoch + 1
                ...
                acc1 = test_with_pyreader(test_exe, test_pyreader, test_fetch_list,
                                        test_metrics, log_interval, save_model_name)
                if acc1 > best_acc1:
                    logger.info('[TRAIN] best_acc1 {} , last_best_acc1: {}'.
                        format(acc1, best_acc1))
                    #print('best_acc1= ',str(acc1),' last_best_acc1= ',str(best_acc1))
                    best_acc1 = acc1
                    save_model(exe, train_prog, save_dir+'_best')
                else:
                    logger.info('[TRAIN] acc1 {} , best_acc1: {}'.
                        format(acc1, best_acc1))
        finally:
            epoch_period = []
...
def save_model(exe, program, save_dir):
